...
-- 创建一个用于隐藏和显示GUI的按钮，使用TextButton和ImageLabel
local ToggleButton = Instance.new("TextButton")
local ToggleImage = Instance.new("ImageLabel")

ToggleImage.Image = "rbxassetid://128885038925647" -- 替换为您的图片资产ID
ToggleImage.Parent = ToggleButton
ToggleImage.Size = UDim2.new(0, 50, 0, 50) -- 设置图片大小
ToggleImage.BackgroundTransparency = 1 -- 没有背景
ToggleImage.ImageTransparency = 0 -- 图片不透明

ToggleButton.Size = UDim2.new(0, 50, 0, 50) -- 设置按钮大小
ToggleButton.Position = UDim2.new(1, -50, 1, -50) -- 设置按钮位置，位于右下角
ToggleButton.Parent = Main -- 假设Main是GUI的主框架

-- 按钮点击事件
ToggleButton.MouseButton1Click:Connect(function()
    isVisible = not isVisible
    DropShadowHolder.Visible = isVisible -- 假设DropShadowHolder是需要隐藏和显示的GUI部分
    GuiFunc:UpdateToggleState(isVisible)
end)

function GuiFunc:UpdateToggleState(state)
    if state then
        ToggleImage.Image = "rbxassetid://128885038925647" -- 替换为您的开启图片资产ID
    else
        ToggleImage.Image = "rbxassetid://12888503892564" -- 替换为您的关闭图片资产ID
    end
end
