...
local Window = WindUI:CreateWindow({
    ...
    KeySystem = { -- 创建密钥系统
        Key = "1", -- 密钥
        Note = "密钥是 1", -- 注释
        URL = "https://github.com/Footagesus/WindUI", -- 获取密钥的URL（例如：Discord）
...
-- Creating tabs
local Tabs = {
    Home = Window:AddTab({ Title = "首页", Icon = "home" }),
    Main = Window:AddTab({ Title = "主要", Icon = "code" }),
    Teleports = Window:AddTab({ Title = "传送点", Icon = "compass" }),
    Misc = Window:AddTab({ Title = "其它", Icon = "file-text" }),
    Fun = Window:AddTab({ Title = "乐趣", Icon = "coffee" }),
    Webhook = Window:AddTab({ Title = "钩", Icon = "clock" }),
    Premium = Window:AddTab({ Title = "高级", Icon = "diamond" }),
    ...
    spawn(function()
        while WebhookLog do
            ...
            local Embed = {
                ...
                color = 0x8B26BB,
                fields = {
                    { name = '玩家档案', value = playerProfileUrl },
                    { name = '', value = '', },
                    { name = 'C$ - 钱💸', value = '```' .. MoneyPlayer .. '```', inline = true },
                    { name = '钓鱼水平🎣', value = '```' .. LvlPlayer .. '```', inline = true },
                    { name = '', value = '', },
                    { name = '当前的当地时间', value = formattedLocalTime },
                },
...
do
    local _25ms=Tabs.Home:AddButton({
        Title = "加入群聊",
        Description = "群聊:259461151",
        Callback = function()
            setclipboard("259461151")
        end
    })
    local section = Tabs.Home:AddSection("更改日志:")
    ...
    local section = Tabs.Main:AddSection("钓鱼")
    ...
    local autoReelCastShakeT = Tabs.Main:AddToggle("autoReelCastShakeT", {Title = "自动 Fish", Default = false })
    ...
    local AntiAfk3 = Tabs.Main:AddToggle("AntiAfk3", {Title = "反-AFK", Description = "防止玩家被标记为AFK", Default = false })
    AntiAfk3:OnChanged(function(Value)
        AntiAfk = AntiAfk3.Value
        AntiAfk2()
    end)

    local section = Tabs.Main:AddSection("附加的")

    local AutoSellF = Tabs.Main:AddToggle("AutoSellF", {Title = "自动卖鱼", Default = false })
    ...
    local section = Tabs.Misc:AddSection("评估师")
    local ToggleAutoApprari = Tabs.Misc:AddToggle("ToggleAutoApprari", {Title = "自动评估师", Description = "需要拿鱼并进入 / 靠近穆索伍德", Default = false })
    ToggleAutoApprari:OnChanged(function()
        AutoAppraiser = ToggleAutoApprari.Value
        Appraise()
    end)

    local section = Tabs.Misc:AddSection("万圣节🎃")
    ...
    local section = Tabs.Misc:AddSection("FPS服务")
    ...
    local section = Tabs.Fun:AddSection("玩家")
    ...
    local section = Tabs.Webhook:AddSection("Webhook统计信息")
    ...
    InputWebhook:OnChanged(function()
        print("URL改变了:", InputWebhook.Value)
    end)
...
InterfaceManager:SetFolder("xgoHub")
SaveManager:SetFolder("xgoHub/Fisch")
