...
function Luna:Window(WindowSettings)
	...
	function Window:Tab(TabSettings)
		...
		-- Section
		function Tab:Section(name : string)
			...
			-- Button
			function Section:Button(ButtonSettings)
				TabPage.Position = UDim2.new(0,0,0,28)
			...
			-- Label
			function Section:Label(LabelSettings)
				TabPage.Position = UDim2.new(0,0,0,28)
			...
			-- Paragraph
			function Section:Paragraph(ParagraphSettings)
				TabPage.Position = UDim2.new(0,0,0,28)
			...
			-- Slider
			function Section:Slider(SliderSettings, Flag)
				TabPage.Position = UDim2.new(0,0,0,28)
			...
			-- Bind
			function Section:Bind(BindSettings, Flag)
				TabPage.Position = UDim2.new(0,0,0,28)
			...
			-- Dynamic Input
			function Section:Input(InputSettings, Flag)
				TabPage.Position = UDim2.new(0,0,0,28)
			...
			-- Dropdown
			function Section:Dropdown(DropdownSettings, Flag)
				TabPage.Position = UDim2.new(0,0,0,28)
			...
			-- Color Picker
			function Section:ColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				TabPage.Position = UDim2.new(0,0,0,28)
		...
		-- Label
		function Tab:Label(LabelSettings)
		...
		-- Slider
		function Tab:Slider(SliderSettings, Flag)
			local SliderV = { IgnoreConfig = false, Class = "Slider", Settings = SliderSettings }
		...
		-- Toggle
		function Tab:Toggle(ToggleSettings, Flag)    
			local ToggleV = { IgnoreConfig = false, Class = "Toggle" }
		...
		-- Bind
		function Tab:Bind(BindSettings, Flag)
			local BindV = { Class = "Keybind", IgnoreConfig = false, Settings = BindSettings, Active = false }
		...
		-- Dynamic Input
		function Tab:Input(InputSettings, Flag)
			local InputV = { IgnoreConfig = false, Class = "Input", Settings = InputSettings }
		...
		-- Dropdown
		function Tab:Dropdown(DropdownSettings, Flag)
			local DropdownV = { IgnoreConfig = false, Class = "Dropdown", Settings = DropdownSettings}
		...
		-- Color Picker
		function Tab:ColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
			local ColorPickerV = {IgnoreClass = false, Class = "Colorpicker", Settings = ColorPickerSettings}
		...
		function Tab:BuildConfigSection()
			if isStudio then
				Tab:Label({Text = "Config system unavailable. (Environment isStudio)", Style = 3})
		...
		function Tab:BuildThemeSection()
			...
			Tab:Section("Custom Editor")

			local c1cp = Tab:ColorPicker({
				Name = "Color 1",
...
if isStudio then
	...
	Window:HomeTab()
	local bleh =Tabs.Debug:ColorPicker()
	Tabs.Debug:Button({
		Callback = function()
			bleh:Set({
	...
	Tabs.Main:Section("Section Example")
	Tabs.Main:Button({
		Name = "Button Example!",
	...
	Tabs.Main:Bind({
		Name = "Bind Example",
		Description = "Btw Using CreateKeybind is deprecated, use Bind For Future Binds :)",
		CurrentKeybind = "Q",
	...
	Tabs.Main2:Section("The Elements Here Are To Show Unique Features")
	Tabs.Main2:Toggle({
	...
	local s = Tabs.Premium:Section("You can add elements inside section too")
	s:Button()
	s:Label()
	s:Divider()
	s:Dropdown()
...
--[[if isStudio then
    window = Luna:Window({LoadingEnabled = false})
    t1 = window:Tab()
    t2 = window:Tab({ Name = "Tab 2", Icon = "location_searching"})
    Luna:Notification({ 
        Title = "Welcome to Luna",
        Icon = "sparkle",
        ImageSource = "Material",
    ...
    t1:Section()
    local btn = t1:Button({Callback = "", Description = "This Is A Description"})
    local l = t1:Label({ Style = 2})
    local l2 = t1:Label({ Text = "Another Label" })
    t2:Button({ Callback = function() 
        l:Destroy()
        l2:Set("New Text")
    end})
    t2:Label({Style = 3})
    t1:Paragraph({Text = "Single String"})
    t1:Paragraph({Text = "Welcome to the Luna Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. Luna was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Luna Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Luna Today at " .. website})
    s = t2:Slider({ Callback = function(v) print(v) end })	
    t1:Button({ Callback = function()
        s:Set({Name = "new name", Callback = ""})
        wait(5)
        s:Destroy()
    end})
    t1:ColorPicker()
    local toggle = t1:Toggle({Name = "test", Description = "test", CurrentValue = true, Callback = ""}, "toggle2")
    t1:Toggle({Callback = function(Value) toggle:Destroy() print(Value) end})
    local bind = t2:Bind({Name = "test", Description = "test", CurrentBind = "E", HoldToInteract = false, Callback = ""})
    ...
    local input = t2:Input({Name = "test",ClearTextAfterFocusLost = false, Description = "Numbers only, 8 max characters and enter required.",MaxCharacters = 8 ,Numeric = true, Enter = true, Callback = ""})
    t2:Input({Callback = function(text) print(text) end})
    local d = t1:Dropdown({Name = "test", Options = {"Apples", "Bananas", "Strawberries", "Elixir"}, Description = "MultiOptions", MultipleOptions = true, Callback = function(t) print(t) end, CurrentOption = {"Apples", "Elixir"}})
    t1:Dropdown({Callback = function(t) print(unpack(t)) end})
    t1:Dropdown({Description = "Special Type - Player", Callback = "", SpecialType = "Player"})
end]]--
