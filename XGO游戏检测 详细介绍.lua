...
local function main()
    -- 系统发送消息    
    -- 假设你已经有了audioId和playSound函数

    -- 发送带有按钮的通知
    local notification = game:GetService("StarterGui"):SendNotification({
    ...
    -- 发送带有按钮的通知
    notification = game:GetService("StarterGui"):SendNotification({
        Title = "开启检测...";
        Text = "🐾🐾🐾……";
        Icon = "rbxthumb://type=Asset&id=120611289434746&w=150&h=150";
        Duration = 1.5;
    ...
    -- 发送带有按钮的通知，并处理按钮点击事件
    local function sendNotificationWithTitleAndButtons(title, text, icon, duration, button1, button2)
        local notification = game:GetService("StarterGui"):SendNotification({
            Title = title,
            Text = text,
            Icon = icon,
        ...
        -- 为通知按钮设置回调
        notification.Dismissed:Connect(function(player, choice)
            if choice == 1 then
                -- 用户点击“是”，执行检测逻辑
                print("检测已开启")
                -- 这里添加你的检测逻辑代码
    ...
    sendNotificationWithTitleAndButtons("开启检测...", "🐾🐾🐾……", "rbxthumb://type=Asset&id=120611289434746&w=150&h=150", 1.5, "是", "不是")
end
