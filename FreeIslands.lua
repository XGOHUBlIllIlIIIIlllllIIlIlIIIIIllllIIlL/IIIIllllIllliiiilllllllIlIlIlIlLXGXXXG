print("脚本源代码发布于 17.03.2024 （日/月/年，因为Islands死了！")
warn("此脚本从现在起停止更新！")
for i = 1, 50 do
  print("Nekohub在顶部 | discord.gg/MbsxuDEzgT")
end
-- 这是一个相当旧的脚本，所以不要拿源代码类型开玩笑（它比90%的Roblox社区都要老） --
...
function SendNotification(Title, Text)
	game:GetService("StarterGui"):SetCore("SendNotification",{
		Title = Title, -- 必需
		Text = Text, -- 必需
		Icon = NotificationIcon -- 可选
	})
end	

SendNotification("欢迎！", "欢迎来到 "..FileName .. " " .. ScriptVersion.."！")

local PremiumText1 = "Premium仅需2美元终身。在discord.gg/MbsxuDEzgT购买")
...
DebugCheck(0,"完成远程：",Finishremote)

if WerfRemote then
	DebugCheck(0,"Werf远程：",WerfRemote)
end

UpdateRemote("FishFarmFinishRemote", Finishremote) -- 2
SendNotification("[安全]", "检查 1/42 完成！")
UpdateRemote("AngelRemote", WerfRemote) -- 2
SendNotification("[安全]", "检查 2/42 完成！")

CreateFile("/哈希系统/FishFarmFinishRemote", Finishremote, false)
SendNotification("[安全]", "检查 3/42 完成！")
CreateFile("/哈希系统/AngelRemote", WerfRemote, false)
SendNotification("[安全]", "检查 4/42 完成！")
Tool.Parent = game.Players.LocalPlayer.Backpack
Tool:Destroy()
...
function Updateinv(v)
	if v:FindFirstChild("sword") then
		if _G.KillRemoteHash == nil or _G.KillRemoteHash == "" or _G.KillRemoteHash == " " then
			...
			SendNotification("[安全]", "检查 5/42 完成！")
			UpdateRemote("KillRemoteHashName", MOBLEFT) -- 2
			SendNotification("[安全]", "检查 6/42 完成！")
			_G.KillRemoteHash = MOBRIGHT
			SendNotification("[安全]", "检查 7/42 完成！")
		end
	end
end
...
UpdateRemote("KillRemote", mobRemoteName) -- 2
SendNotification("[安全]", "检查 8/42 完成！")
UpdateRemote("KillRemoteHashName", MOBLEFT) -- 2
SendNotification("[安全]", "检查 9/42 完成！")
_G.KillRemoteHash = MOBRIGHT
...
CreateFile("/哈希系统/KillRemote", mobRemoteName, false)
SendNotification("[安全]", "检查 10/42 完成！")
CreateFile("/哈希系统/KillRemoteHashName", MOBLEFT, false)
SendNotification("[安全]", "检查 11/42 完成！")
CreateFile("/哈希系统/KillRemoteHash", MOBRIGHT or _G.KillRemoteHash, false)
SendNotification("[安全]", "检查 12/42 完成！")
...
SendNotification("[安全]", "检查 13/42 完成！")
_G.TOOL_PICKUPHash = TOOLRIGHT
SendNotification("[安全]", "检查 14/42 完成！")

CreateFile("/哈希系统/TOOL_PICKUPHashData", ToolLEFT, false)
SendNotification("[安全]", "检查 15/42 完成！")
CreateFile("/哈希系统/TOOL_PICKUPHash", TOOLRIGHT, false)
SendNotification("[安全]", "检查 16/42 完成！")
...
print("调试 4")
			...
			function CropUpdate()
				...
				if remleft and remright then
					...
					game.Players.LocalPlayer:Kick("错误：NCD1")
end
end
task.spawn(function()
	CropUpdate()
...
SendNotification("[安全]", "检查 17/42 完成！")
_G.CropHash = CROPright
SendNotification("[安全]", "检查 18/42 完成！")

UpdateRemote("TreeHashData", HITleft) -- 2
SendNotification("[安全]", "检查 19/42 完成！")
_G.TreeHash = HITright
SendNotification("[安全]", "检查 20/42 完成！")
UpdateRemote("BlockHitHashData", HITleft) -- 2
SendNotification("[安全]", "检查 21/42 完成！")
_G.BlockHitHash = HITright
SendNotification("[安全]", "检查 22/42 完成！")

CreateFile("/哈希系统/CropHashData", CROPleft, false)
SendNotification("[安全]", "检查 23/42 完成！")
CreateFile("/哈希系统/CropHash", CROPright, false)
SendNotification("[安全]", "检查 24/42 完成！")

print("调试 7")

CreateFile("/哈希系统/TreeHashData", HITleft, false)
SendNotification("[安全]", "检查 25/42 完成！")
CreateFile("/哈希系统/TreeHash", HITright, false)
SendNotification("[安全]", "检查 26/42 完成！")
CreateFile("/哈希系统/BlockHitHashData", HITleft, false)
SendNotification("[安全]", "检查 27/42 完成！")
CreateFile("/哈希系统/BlockHitHash", HITright, false)
SendNotification("[安全]", "检查 28/42 完成！")
...
UpdateRemote("FlowerCollect", "client_request_1") -- 1
SendNotification("[安全]", "检查 29/42 完成！")
UpdateRemote("PetCollect", "CLIENT_PET_ANIMAL")
SendNotification("[安全]", "检查 30/42 完成！")
UpdateRemote("BlockRemote", "CLIENT_BLOCK_HIT_REQUEST")
SendNotification("[安全]", "检查 31/42 完成！")
-- UpdateRemote("SendMailRemote", "NINGI(HUINUH(UIN(IH(HUUI")
SendNotification("[安全]", "检查 32/42 完成！")
UpdateRemote("SpiritRemote", "gecqaLhbvAbyjo/qDgqEvuQzyryuBnlzh")
SendNotification("[安全]", "检查 33/42 完成！")

print("BlockLeft:", BlockLeft)

UpdateRemote("CropPlaceHashData", BlockLeft) -- 2
SendNotification("[安全]", "检查 34/42 完成！")
_G.CropPlaceHash = BlockRight -- 2
SendNotification("[安全]", "检查 35/42 完成！")
_G.CropPlaceH1 = BlockLeft
SendNotification("[安全]", "检查 36/42 完成！")
UpdateRemote("BlockPlaceHashData", BlockLeft) -- 2
SendNotification("[安全]", "检查 37/42 完成！")
_G.BlockPlaceHash = BlockRight -- 2
SendNotification("[安全]", "检查 38/42 完成！")

print("调试 11")

CreateFile("/哈希系统/CropPlaceHashData", BlockLeft, false)
SendNotification("[安全]", "检查 39/42 完成！")
CreateFile("/哈希系统/CropPlaceHash", BlockRight, false)
SendNotification("[安全]", "检查 40/42 完成！")
CreateFile("/哈希系统/BlockPlaceHashData", BlockLeft, false)
SendNotification("[安全]", "检查 41/42 完成！")
CreateFile("/哈希系统/BlockPlaceHash", BlockRight, false)
SendNotification("[安全]", "检查 42/42 完成！")
end
Update()
end

SendNotification("[安全]", "正在扫描游戏...")
...
local minDistance = 3 -- 触发传送到目标的最小距离
local maxIterations = 3 -- 允许的最大迭代次数，如果没有进展

DebugCheck(0,"初始位置: " .. tostring(myPart.Position))
DebugCheck(0,"目标位置: " .. tostring(targetPosition))
	...
	while (myPart.Position - targetPosition).magnitude > minDistance do
		...
		for i = 1, steps do
			...
			MiniTpV2(newPos)
			DebugCheck(0,"当前位置: " .. tostring(myPart.Position))
DebugCheck(0,"在半径内！") -- 当玩家在半径内时的输出
wait(waitTime)
...
DebugCheck(0,"最终位置: " .. tostring(myPart.Position))
DebugCheck(0,"已到达目标位置！")
	UseFloat = false
end
...
if DeveloperVersion == false then
    if DidKey == true then
        Window = Fluent:CreateWindow({
            Title = "Islands.God "..ScriptVersion.." [免费！] "..holidayMessage,
            SubTitle = "by NtOpenProcess 和 soldo_io", -- neverloseyoursmile_88
            TabWidth = 160,
            Size = UDim2.fromOffset(580, 460),
            Acrylic = true, -- 模糊效果可能会被检测到，设置为false将完全禁用模糊
            Theme = "Dark",
            MinimizeKey = Enum.KeyCode.LeftControl -- 当没有最小化键绑定时使用
        })

        Tabs = {
            Main = Window:AddTab({ Title = "主界面", Icon = "sword" }),
            Teleports = Window:AddTab({ Title = "传送点", Icon = "bus" }),
            BlockPrinter = Window:AddTab({ Title = "方块打印机", Icon = "printer" }),
            Player = Window:AddTab({ Title = "玩家", Icon = "PlayerIcon" }),
            VendingSniper = Window:AddTab({ Title = "售卖机狙击手", Icon = "file-search" }),
            BypassFun = Window:AddTab({ Title = "绕过.乐趣", Icon = "server" }),
            Other = Window:AddTab({ Title = "其他", Icon = "sidebar" }),
            Settings = Window:AddTab({ Title = "设置", Icon = "settings" }),
            Premium = Window:AddTab({ Title = "高级", Icon = "star" })
		}

	else

		Window = Fluent:CreateWindow({
    Title = "Islands.God "..ScriptVersion.." [高级版！] "..holidayMessage,
    SubTitle = "由 NtOpenProcess 和 soldo_io 开发", -- neverloseyoursmile_88
    TabWidth = 160,
    Size = UDim2.fromOffset(580, 460),
    Acrylic = true, -- 模糊效果可能会被检测到，设置为false将完全禁用模糊
    Theme = "Dark",
...
Tabs = {
	...
	Window = Fluent:CreateWindow({
    Title = "Islands.God "..ScriptVersion.." [开发者模式！] "..holidayMessage,
    SubTitle = "由 NtOpenprocess 和 soldo_io 开发", -- neverloseyoursmile_88
    TabWidth = 160,
    Size = UDim2.fromOffset(580, 460),
    Acrylic = true, -- 模糊效果可能会被检测到，设置为false将完全禁用模糊
...
Tabs = {
    ...
    Other = Window:AddTab({ Title = "其他", Icon = "sidebar" }),
    Settings = Window:AddTab({ Title = "设置", Icon = "settings" }),
    DeveloperTab = Window:AddTab({ Title = "开发者", Icon = "" }),
}
end 
...
if DidKey == true then
    ...
    local H = Tabs.Premium:AddParagraph({
        Title = "为什么选择高级版？",
        Content = "选择高级版，您将获得更快的更新和测试版功能！"
    })

    local H = Tabs.Premium:AddParagraph({
        Title = "如何购买？",
        Content = "加入我们的Discord服务器discord.gg/MbsxuDEzgT并提交工单。"
    })

    local H = Tabs.Premium:AddParagraph({
        Title = "支持哪些支付方式？",
        Content = "您可以通过以下方式购买脚本：\n1.Paypal\n2.Robux"
    })

    local H = Tabs.Premium:AddParagraph({
        Title = "价格是多少？",
        Content = "1.Paypal: 2美元\n2.Robux:800R$"
    })
